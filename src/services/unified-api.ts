...
class UnifiedApiService {
   ...
   */
  configureCertificates(config: CertificateConfig) {
    this.certificateConfig = config;
    this.mtlsEnabled = config.enableMtls || false;
    console.log('🔐 Certificate configuration updated', { 
      hasCert: !!config.clientCert,
      hasKey: !!config.clientKey,
  ...
  async smartRequest<T>(
    ...
    // Strategy 0: Try mTLS if enabled and configured
    if (this.mtlsEnabled && this.certificateConfig.clientCert) {
      try {
        console.log(`🔐 Attempting mTLS API for ${endpoint} (attempt ${attempts + 1})`);
        const mtlsOptions = {
          ...options,
          headers: {
            ...options.headers,
            'X-Client-Cert': this.certificateConfig.clientCert,
            'X-Client-Key-Hash': this.certificateConfig.clientKey ? btoa(this.certificateConfig.clientKey.substring(0, 64)) : ''
          }
        };
        
        const data = await api.request<T>(endpoint, mtlsOptions);
        ...
        console.warn(`❌ mTLS API failed for ${endpoint}:`, error);
        lastError = error;
        attempts++;
      }
    }
    ...
    if (!this.preferCloudflareApi) {
      try {
        ...
        const data = await api.request<T>(endpoint, options);
  ...
  getStatus() {
    return {
      ...
      mtlsEnabled: this.mtlsEnabled,
      hasCertificates: {
        clientCert: !!this.certificateConfig.clientCert,
        clientKey: !!this.certificateConfig.clientKey,
        caCert: !!this.certificateConfig.caCert
      }
    };
  }
   ...
   */
  async testCertificateAuth(): Promise<ApiResponse> {
    if (!this.certificateConfig.clientCert) {
      return {
        success: false,
        error: 'No client certificate configured',
        strategy: 'mtls'
      };
    }
    ...
    try {
      ...
      const result = await this.smartRequest('/auth/certificate-test');
      return result;
    } catch (error) {
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Certificate test failed',
        strategy: 'mtls'
      };
    }
  }
